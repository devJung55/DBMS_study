...
SELECT PLAYER_NAME || '의 영어 이름은 ' || E_PLAYER_NAME || '이다.' AS "자기 소개" FROM PLAYER;

/*PLAYER_NAME의 포지션은 POSITION입니다.*/
SELECT PLAYER_NAME || '의 포지션은 ' || NVL("POSITION", '미정') || '입니다.' AS "자기 소개" FROM PLAYER;
...
SELECT "POSITION", MIN(WEIGHT) "평균 몸무게", AVG(HEIGHT) "평균 키" FROM PLAYER
WHERE "POSITION" IS NOT NULL AND WEIGHT >= 80
GROUP BY "POSITION"
...
SELECT "POSITION", AVG(HEIGHT)"포지션별 평균 키", (SELECT AVG(HEIGHT) FROM PLAYER) "전체 평균 키" FROM PLAYER
WHERE "POSITION" IS NOT NULL
GROUP BY "POSITION";
